...
int parse_url(string url,url_t* url_info){
    const int http_prefix_len=7;
    if(strncasecmp(url,"http://",http_prefix_len)!=0){
        fprintf(stderr, "url error, not http protocol: %s\n", url);
        return -1;
    }
    char* host_ptr=url+http_prefix_len;
    char* port_ptr=strchr(host_ptr,':');
    char* path_ptr=strchr(host_ptr,'/');
    if(path_ptr==NULL){
        fprintf(stderr, "url error, no path: %s\n", url);
        return -1;
    }
    if(port_ptr==NULL){
        *path_ptr='\0';
        strcpy(url_info->hostname,host_ptr);
        strcpy(url_info->port,"80");
        *path_ptr='/';
        strcpy(url_info->path,path_ptr);
    }
    else{
        *port_ptr='\0';
        strcpy(url_info->hostname,host_ptr);
...
int parse_header(rio_t* client_rio_p, string header_info, string host){
    ...
    while(1){
        ...
        if(strncasecmp(buf,"Proxy-Connection:",strlen("Proxy-Connection:"))==0){
            continue;
        }
...
void process(rio_t* client_rio_p,string url){
    ...
    if(parse_url(url,&url_info)<0){
        fprintf(stderr, "parse url error: %s\n", url);
        return;
    }
    string header_info;
    ...
    if(server_fd<0){
        fprintf(stderr, "open client fd error: %s\n", url);
        return;
    }
    rio_t server_rio;
    ...
    while((resp_current=rio_readnb(&server_rio,file_cache,MAXLINE))){
        if (resp_current < 0) {
